...
## Observing the dataframe again
```{r}
str(songs)
```
Notice the brackets with the `genre` column. It warrants further investigation

```{r}
unique(songs$genre)
```

All `genre` observations have square brackets (`[]`), which have to be removed
